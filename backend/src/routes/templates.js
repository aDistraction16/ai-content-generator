...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const templates = await db
      ...
      .where(or(eq(contentTemplates.userId, userId), eq(contentTemplates.isPublic, true)))
  ...
  try {
    ...
    const template = await db
      ...
      .where(and(
        ...
        or(eq(contentTemplates.userId, userId), eq(contentTemplates.isPublic, true))
      ))
      .limit(1);
...
router.post('/', requireAuth, async (req, res) => {
  try {
    const userId = req.user.id;
    const { name, description, contentType, platformTarget, template, variables } = req.body;

    // Validate required fields
    if (!name || !template || !contentType || !platformTarget) {
      return res.status(400).json({ 
        message: 'Name, template, content type, and platform target are required' 
      });
    }
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const existingTemplate = await db
      .select()
      .from(contentTemplates)
      .where(and(eq(contentTemplates.id, parseInt(id)), eq(contentTemplates.userId, userId)))
      .limit(1);
    ...
    const updatedTemplate = await db
      ...
      .set({
        ...
        variables: JSON.stringify(variables || JSON.parse(existing.variables || '[]')),
        updatedAt: new Date()
      })
      .where(and(eq(contentTemplates.id, parseInt(id)), eq(contentTemplates.userId, userId)))
      .returning();
...
router.post('/generate', requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to generate content from template' });
  }
});
