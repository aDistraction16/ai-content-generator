...
export const calculateEngagementMetrics = (contentItem) => {
  const { contentType, platformTarget, wordCount, characterCount, generatedText } = contentItem;
  ...
  const hasCallToAction = /\b(click|visit|check|learn|discover|try|get|download|sign up|subscribe)\b/gi.test(generatedText);
  const hasEmojis = /[\u{1F600}-\u{1F64F}|\u{1F300}-\u{1F5FF}|\u{1F680}-\u{1F6FF}|\u{1F1E0}-\u{1F1FF}]/gu.test(generatedText);
  ...
  const potentialReach = Math.round(baseEngagement * reachMultiplier * qualityMultiplier * lengthMultiplier);
  ...
  const engagementScore = Math.round((estimatedEngagements / potentialReach) * 100);
...
export const getAdvancedAnalytics = async (userId, startDate, endDate) => {
  try {
    // Check cache first
    const cachedAnalytics = await ContentCache.getCachedAnalytics(userId, startDate, endDate);
    if (cachedAnalytics) {
    ...
    const contentTrend = previousTotal > 0 ? ((currentTotal - previousTotal) / previousTotal) * 100 : 0;
    ...
    await ContentCache.setCachedAnalytics(userId, startDate, endDate, result, 1800);
...
export const getContentPerformanceScores = async (userId) => {
  try {
    ...
    const avgScore = scoredContent.reduce((sum, item) => sum + item.performanceScore, 0) / scoredContent.length;
    const topPerformers = scoredContent.filter(item => item.performanceScore > avgScore + 10);
    const needsImprovement = scoredContent.filter(item => item.performanceScore < avgScore - 10);
