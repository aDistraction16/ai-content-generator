...
import { getAdvancedAnalytics, getContentPerformanceScores } from './analytics.js';
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const summaryData = [
        ...
        ['Average Reach per Content:', totalContent > 0 ? `${Math.round(totalReach / totalContent).toLocaleString()} people` : '0'],
      ];
      ...
      Object.entries(contentByType).forEach(([type, count]) => {
        const percentage = totalContent > 0 ? ((count / totalContent) * 100).toFixed(1) : '0';
        doc.fontSize(11)
           .fillColor('#374151')
           .text(`${type.replace('_', ' ').toUpperCase()}: ${count} pieces (${percentage}%)`, { indent: 20 });
      });
      ...
      if (contentList && contentList.length > 0) {
        ...
        contentList.slice(0, 15).forEach((item, index) => {
          ...
          doc.fontSize(10)
             .fillColor('#6B7280')
             .text(`   Type: ${item.contentType} | Created: ${new Date(item.createdAt).toLocaleDateString()}`)
             .text(`   Potential Reach: ${(item.potentialReachMetric || 0).toLocaleString()} people`);
          
          if (item.scheduledAt) {
            doc.text(`   Scheduled: ${new Date(item.scheduledAt).toLocaleString()}`);
          }
      ...
      doc.fontSize(8)
         .fillColor('#9CA3AF')
         .text(`Report generated by AI Content Generator`, 50, doc.page.height - 30, { align: 'center' });
...
export const generateEnhancedPDFReport = async (reportData, userId) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const analytics = await getAdvancedAnalytics(userId, periodStart, periodEnd);
      ...
      const summary = [
        ...
        `Top Performing Content Type: ${performance.insights.bestPerformingType?.type || 'N/A'}`,
        `Best Platform: ${performance.insights.bestPerformingPlatform?.platform || 'N/A'}`
      ];
      ...
      if (analytics.trends) {
        const contentTrendText = analytics.trends.contentTrend >= 0 
          ? `+${analytics.trends.contentTrend.toFixed(1)}% increase`
          : `${analytics.trends.contentTrend.toFixed(1)}% decrease`;
        
        const reachTrendText = analytics.trends.reachTrend >= 0 
          ? `+${analytics.trends.reachTrend.toFixed(1)}% increase`
          : `${analytics.trends.reachTrend.toFixed(1)}% decrease`;

        doc.fontSize(11)
           .fillColor('#374151')
           .text(`Content Generation Trend: ${contentTrendText} vs previous period`, { indent: 20 })
           .text(`Reach Trend: ${reachTrendText} vs previous period`, { indent: 20 });
      }
      ...
      if (analytics.contentByType && analytics.contentByType.length > 0) {
        analytics.contentByType.forEach(item => {
          const percentage = totalContent > 0 ? ((item.count / totalContent) * 100).toFixed(1) : '0';
          doc.fontSize(11)
             .fillColor('#374151')
             .text(`${item.contentType} (${item.platformTarget || 'General'}): ${item.count} pieces (${percentage}%)`, { indent: 20 });
        });
      }
      ...
      if (performance.contentScores && performance.contentScores.length > 0) {
        ...
        topContent.forEach((item, index) => {
          ...
          doc.fontSize(9)
             .fillColor('#6B7280')
             .text(`   Score: ${item.performanceScore}% | Type: ${item.contentType} | Reach: ${item.potentialReachMetric || 0}`, { indent: 20 });
          ...
          if (item.qualityFactors) {
            ...
            if (item.qualityFactors.optimalLength) factors.push('Optimal Length');
            
            if (factors.length > 0) {
              doc.text(`   Quality Factors: ${factors.join(', ')}`, { indent: 20 });
            }
          }
      ...
      if (doc.y < 600) { // Check if there's enough space
        addSectionHeader(doc, 'Content Details');
        
        contentList.slice(0, 10).forEach((item) => { // Limit to 10 items
          doc.fontSize(9)
             .fillColor('#1F2937')
          ...
          doc.fontSize(8)
             .fillColor('#6B7280')
             .text(`Created: ${new Date(item.createdAt).toLocaleDateString()} | Reach: ${item.potentialReachMetric || 0}`, { indent: 20 });
      ...
      doc.fontSize(8)
         .fillColor('#9CA3AF')
         .text(`Report generated by AI Content Generator for ${userEmail}`, 50, doc.page.height - 30, { align: 'center' });
...
function generateInsights(analytics, performance) {
  ...
  if (analytics.overview?.total > 0) {
    if (analytics.trends?.contentTrend > 10) {
      insights.push('Great momentum! Your content generation has increased significantly.');
    } else if (analytics.trends?.contentTrend < -10) {
      insights.push('Consider increasing your content generation frequency for better engagement.');
    }
  }

  // Performance insights
  if (performance.insights?.averageScore > 75) {
    insights.push('Excellent content quality! Your average performance score is above 75%.');
  } else if (performance.insights?.averageScore < 50) {
    insights.push('Focus on content quality. Try adding more engaging elements like questions and call-to-actions.');
  }

  // Platform insights
  if (performance.insights?.bestPerformingPlatform) {
    insights.push(`${performance.insights.bestPerformingPlatform.platform} shows the best performance. Consider focusing more content on this platform.`);
  }

  // Content type insights
  if (performance.insights?.bestPerformingType) {
    insights.push(`${performance.insights.bestPerformingType.type} content performs best with an average score of ${performance.insights.bestPerformingType.averageScore}%.`);
  }

  // Reach insights
  if (analytics.trends?.reachTrend > 20) {
    insights.push('Your content reach is growing rapidly! Keep up the current content strategy.');
  }

  // Default insights if none generated
  if (insights.length === 0) {
    insights.push('Continue creating consistent, quality content to improve your metrics.');
    insights.push('Experiment with different content types and platforms to find what works best.');
    insights.push('Use hashtags, questions, and call-to-actions to boost engagement.');
  }
