  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .text(`Report Period: ${periodStart} - ${periodEnd}`, {
          align: "center",
        });
      ...
      // Summary Section
      doc.fontSize(18).fillColor("#333").text("SUMMARY", { underline: true });
      ...
      doc.text(
        `Blog Posts: ${contentByType.blog_post || 0}`,
        50,
        summaryY + 20
      );
      doc.text(
        `Social Captions: ${contentByType.social_caption || 0}`,
        300,
        summaryY + 20
      );
      ...
      if (contentList && contentList.length > 0) {
        ...
        contentList.forEach((content, index) => {
          ...
          if (content.platformTarget) {
            doc.text(`Platform: ${content.platformTarget}`, {
              continued: false,
            });
          }
          ...
          doc
            ...
            .fillColor("#333")
            .text(
              `Words: ${content.wordCount} | Characters: ${content.characterCount} | ` +
                `Potential Reach: ${content.potentialReachMetric}`,
              { continued: false }
            );
          ...
          doc.fontSize(9).fillColor("#555").text(`Preview: ${preview}`, {
            width: 500,
            align: "justify",
          });
      ...
      // Calculate insights
      const avgWordsPerContent =
        contentList.length > 0
          ? Math.round(
              contentList.reduce((sum, c) => sum + c.wordCount, 0) /
                contentList.length
            )
          : 0;

      const avgReachPerContent =
        contentList.length > 0
          ? Math.round(
              contentList.reduce((sum, c) => sum + c.potentialReachMetric, 0) /
                contentList.length
            )
          : 0;
      ...
      const topPlatform =
        Object.keys(mostUsedPlatform).length > 0
          ? Object.entries(mostUsedPlatform).sort(([, a], [, b]) => b - a)[0][0]
          : "N/A";

      doc.text(`- Average words per content: ${avgWordsPerContent}`);
      doc.text(
        `- Average potential reach per content: ${avgReachPerContent.toLocaleString()}`
      );
      doc.text(`- Most used platform: ${topPlatform}`);
      doc.text(
        `- Content creation frequency: ${(contentList.length / 7).toFixed(
          1
        )} pieces per day`
      );
...
const generateRecommendations = (reportData) => {
  ...
  if (totalContent === 0) {
    recommendations.push(
      "Start creating content to build your presence and engage your audience."
    );
  ...
  if (totalContent < 7) {
    recommendations.push(
      "Consider increasing your content creation frequency to at least one piece per day for better engagement."
    );
  } else if (totalContent > 20) {
    recommendations.push(
      "Great job on consistent content creation! Focus on quality and engagement metrics."
    );
  }
  ...
  if (blogPosts === 0 && socialCaptions > 0) {
    recommendations.push(
      "Consider adding blog posts to provide more in-depth content for your audience."
    );
  } else if (socialCaptions === 0 && blogPosts > 0) {
    recommendations.push(
      "Add social media captions to increase your social presence and engagement."
    );
  }

  // Platform diversity
  const platforms = [
    ...new Set(contentList.map((c) => c.platformTarget).filter(Boolean)),
  ];
  if (platforms.length === 1) {
    recommendations.push(
      "Diversify your content across multiple platforms to reach a broader audience."
    );
  }
  ...
  if (draftContent > scheduledContent && draftContent > 3) {
    recommendations.push(
      "You have many draft pieces. Consider scheduling them to maintain consistent posting."
    );
  }
  ...
  if (avgReach < 100) {
    recommendations.push(
      "Focus on trending topics and engaging content to improve your potential reach."
    );
  }
