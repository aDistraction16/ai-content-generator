/**
 * Provides API functions for managing and generating reports.
 *
 * @remarks
 * This service includes methods for generating PDF reports, downloading reports,
 * retrieving report history, fetching dashboard analytics, sending weekly summary emails,
 * generating enhanced PDF reports, and deleting reports.
 *
 * @example
 * // Generate a PDF report for a specific date range
 * const { message, report, downloadUrl } = await reportsAPI.generatePDFReport({ startDate, endDate });
 *
 * @example
 * // Download a report by ID
 * const pdfBlob = await reportsAPI.downloadReport(reportId);
 *
 * @example
...
export const reportsAPI = {
  ...
  generateEnhancedReport: async (): Promise<void> => {
    try {
      ...
      link.setAttribute(
        "download",
        `enhanced-content-report-${Date.now()}.pdf`
      );
